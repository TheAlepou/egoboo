...
/* ov_callbacks.seek_func expects int: 0 on success, nonzero on failure */
static int sdl_seek_func(void *datasource, ogg_int64_t offset, int whence)
{
    Sint64 result = SDL_RWseek((SDL_RWops*)datasource, (Sint64)offset, whence);
    return (result < 0) ? -1 : 0;
}

/* ov_callbacks.close_func expects int: 0 on success, EOF or nonzero on failure */
static int sdl_close_func_freesrc(void *datasource)
{
    return (SDL_RWclose((SDL_RWops*)datasource) == 0) ? 0 : EOF;
}

static int sdl_close_func_nofreesrc(void *datasource)
{
    /* Don't free; just reset position to start to mimic a benign close. */
    Sint64 result = SDL_RWseek((SDL_RWops*)datasource, 0, RW_SEEK_SET);
    return (result < 0) ? EOF : 0;
}

/* ov_callbacks.tell_func expects long return */
static long sdl_tell_func(void *datasource)
{
    Sint64 pos = SDL_RWtell((SDL_RWops*)datasource);
    if (pos < 0) {
        return -1L;
    }
    if (pos > (Sint64)LONG_MAX) {
